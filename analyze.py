...
# abbreviations = {'television':'tv','motion':'movie','picture':'movie','film':'movie','motion':'limited','picture':'limited'}

# # Dictionary of keywords and what category they correspond to
keywords_dict = {' not win:':3,' win':1,'congratulation':1,' present':2,' announc':2,' gave; award':2,'giv; award':2,' won from':2,'nominee':3,'nominated':3,'nomin':3, 'best speech':4,'best dress':5,'best look':5,'worst dress':6,'worst look':6}
...
# # # keywords_dict = {' present':2,' announc':2,' gave; award':2,'giv; award':2,' won from':2}
...
def find_tweet_award(tweet_lowercase):
	...
	for award in Award_list:
		...
		for word in award.filtered_sentence:
			...
			if word_lowercase not in tweet_lowercase and (word_lowercase not in abbreviations or abbreviations[word_lowercase] not in tweet_lowercase) and word_lowercase != 'best':
				check = False
			else:
				count += 1
...
# 'desired;lose to | lost to;winner'
# def regex_custom(regex_list,filtered_sentence):
# 	for regex in regex_list:
# 		regex_parts = regex.split(';')
# 		keys = regex_parts[1].split('|')
# 		if(regex_parts[0]=='desired'):
...
def find_named_entities(tweet, category, skip_nltk = False, start_word = ''):
	...
	if(category == 3 and entity_list == []):
		...
		for index in range(0,len(filtered_sentence)):
			...
			if(word == ':' or word == 'over' or word == 'are'):
				i = index + 1
				break
		if(i != 0):
			check = True
...
def findNominee(dataframe):
	...
	df = keywordFilter(dataframe, [], ['win over','won over','not win', "didn't win", 'lost','beat out','beating out',
				'lose to','lost to','lose an award','lost an award','should have won'])
...
# 		winner = award.winner
# 		if(winner != ''):
# 			tweetsList = keywordFilter(df,[winner], ['win over','won over','not win', "didn't win", 'lost','beat out','beating out',
# 				'lose to','lost to','lose an award','lost an award','should have won']).to_dict('records')
# 			award.voting_dict[3][winner] = award.voting_dict[1][winner]
# 			for tweet in tweetsList:
# 				print (tweet)
# 				for entity in find_named_entities(tweet['text'], 3, False, winner):
# 					entity = entity.split('/')[0]
# 					if entity in award.voting_dict[3]:
# 						award.voting_dict[3][entity] += tweet['id_str']
# 					else:
...
def findPresenter(df):
	for award in Award_list:
		...
		# award.voting_dict[2] = new_votes_dict
